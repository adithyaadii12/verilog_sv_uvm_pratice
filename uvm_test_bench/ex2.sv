...
class my_driver extends uvm_driver #(my_sequence_item);
  ...
  event drv_done;
  uvm_tlm_fifo#(my_sequence_item) exp_data;

  function new(string name = "", uvm_component parent);
    super.new(name,parent);
//     exp_data = new("exp_data",this);
  endfunction
...
class scoreboard extends uvm_scoreboard;
  ...
  uvm_tlm_analysis_fifo#(my_sequence_item) act_data;
  uvm_tlm_fifo#(my_sequence_item) exp_data;
  my_sequence_item sb_item;
  
  function new(string name = "",uvm_component parent);
    super.new(name,parent);
    act_data = new("act_data",this);
    exp_data = new("exp_data",this);
  endfunction
 ...
 task run_phase(uvm_phase phase);
    forever begin
      exp_data.get(exp_item);
      act_data.get(act_item);

      total_cnt++;

      if (exp_item.addr  == act_item.addr  &&
          exp_item.data  == act_item.data  &&
          exp_item.value == act_item.value &&
          exp_item.enable== act_item.enable) begin
        pass_cnt++;
        `uvm_info("SCOREBOARD",
          $sformatf(
            "PASS addr=%0d | EXP(data=%0d value=%0d en=%0b) ACT(data=%0d value=%0d en=%0b)",
            act_item.addr,
            exp_item.data, exp_item.value, exp_item.enable,
            act_item.data, act_item.value, act_item.enable),
          UVM_LOW)
      end
      else begin
        fail_cnt++;
        `uvm_error("SCOREBOARD",
          $sformatf(
            "FAIL addr=%0d | EXP(data=%0d value=%0d en=%0b) ACT(data=%0d value=%0d en=%0b)",
            act_item.addr,
            exp_item.data, exp_item.value, exp_item.enable,
            act_item.data, act_item.value, act_item.enable))
      end
...
class my_env extends uvm_env;
  ...
  function void connect_phase(uvm_phase phase);
    if (agent.active_passive == UVM_ACTIVE)
      agent.driver.exp_data = sb.exp_data;
    agent.monitor.out_port.connect(sb.act_data.analysis_export);
  endfunction
